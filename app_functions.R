# File containing the function of the app 
...
gene_dna <- function(length, base_probs = c(0.25, 0.25, 0.25, 0.25)){
  ...
  if( abs(sum(base_probs) - 1) > 1e-10 ){
    stop("The base probabilities should sum to 1")}
...
transcribe_dna <- function(dna) {
  if (str_detect(dna, "U")) {
    stop("The DNA sequence must not include any U")
  }
  
  # Use chartr for single-step transcription
  rna <- chartr("TACG", "AUGC", dna)
  return(rna)
}
...
translate_rna <- function(rna){
  if (str_detect(rna, "T")) {
    stop("The DNA sequence must not include any T")
  }
